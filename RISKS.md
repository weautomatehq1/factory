...
| R-006 | IFleet worker rate-limit on Claude API mid-sprint | Opus 5-hour cap hit on long brief; silent code=1 | Sprint stalls; debugging time wasted | 5 | 7 | 4 | 140 | IFleet model routing decision 2026-05-12 (see STAKEHOLDERS.md decision log): sonnet default + opus only on complexity:high; rate-limit detection downshifts; per-project daily rate cap | esme | mitigating | 2026-05-15 |
...
| R-013 | Client interview reveals scope wildly different from expected | Client wants something The Factory can't build (mobile app, ML pipeline) | Wasted interview; awkward conversation; NoGo-2 invoked | 6 | 4 | 3 | 72 | Pre-call screening questionnaire (not yet scoped — planned post-[M-024]); INTAKE flags NoGo categories early; Layer 2 redirects out-of-scope topics | sebas | open | 2026-05-15 |
