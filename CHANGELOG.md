...
### Changed
- **2026-06-21 — Nightly audit fixes** (15 of 20 findings fixed — 1C/7I/7Cos). ROADMAP.md: live git merge conflict in footer resolved (CRITICAL), M-005 adrs field updated from ADR-0006 → ADR-0011, Retell R1 overdue count updated (28→32+ days), 'Next' section heading stale window removed, 'Later' section heading updated to Q3+. README.md: spec-template status corrected from 'Not yet created' to exists (2026-05-16). KPI.md: KR2 day counts updated (11→14 days). ARCHITECTURE.md: Logging/Error Handling owner corrected from AGENTS.md §6 → ARCHITECTURE.md §6. SECURITY.md: 'Protected Paths (the auto-merge gate)' heading simplified to fix GitHub anchor. AGENTS.md: ADR range updated (0010→0011), docs/postmortems/ and docs/templates/ added to tree. NON_GOALS.md: phantom M-026+ corrected to M-024+. CHANGELOG.md: scaffold entry service count corrected (14→13). STATUS.md + .audits/: session recorded, index updated. 5 findings remain open: ADR-0011 incomplete MADR structure (CRITICAL, pre-commit hook blocks fix), SPRINT-2026-05-B missing (CRITICAL), no Q3 OKRs (IMPORTANT), RISKS.md rows stale (IMPORTANT), UBIQUITOUS_LANGUAGE.md path inconsistency (IMPORTANT).
...
- `INTEGRATIONS.md` — 13 third-party services + 4 internal tools registered (corrected from 14; see [Unreleased] 2026-06-07)
