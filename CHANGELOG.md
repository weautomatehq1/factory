...
### Changed
- **2026-06-08 — Nightly audit fixes** (2 findings fixed — 1 IMPORTANT, 1 COSMETIC). ADR-0006 frontmatter normalized: `status: superseded by ADR-0011` → `status: superseded` (non-standard YAML value incompatible with future ADR generators) — closes AUDIT-factory-d4e5f6a7. DECISIONS.md §"How ADRs work here" notes 3 and 4 corrected to describe the canonical `superseded` status value instead of the non-standard `superseded by NNNN` format — closes AUDIT-factory-b8c9d0e1.
